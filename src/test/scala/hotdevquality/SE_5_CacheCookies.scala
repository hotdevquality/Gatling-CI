...
class SE_5_CacheCookies extends Simulation {
	...
	val scn = scenario("SE")
  	...
  	.exec(flushHttpCache)               // deletes the Cache
  	.exec(flushSessionCookies)          // deletes the Session Cookies
  	.exec(flushCookieJar)               // deletes all the Cookies
