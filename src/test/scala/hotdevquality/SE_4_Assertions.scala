...
class SE_4_Assertions extends Simulation {
	...
	setUp(scn.inject(atOnceUsers(1))).protocols(httpProtocol)
  	.assertions(
			global.responseTime.max.lt(1000),															// Asserting whether or not the response time for all the request is at max 1000 seconds
			forAll.responseTime.max.lt(1000),															// Asserting whether or not the response time for each of the request (taken individually) is at max 1000 seconds.
			details("BookFlight").responseTime.max.lt(1000),		// Asserting whether or not the response time for 'BookFlight' request is at max 1000 seconds

			global.successfulRequests.percent.is(100)												// Asserting whether or not 100% of the requests are passing
